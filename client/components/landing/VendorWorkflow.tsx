...
const VendorWorkflow = () => {
  return (
    <section id="vendors" className="py-24 bg-slate-900 relative overflow-hidden">
      {/* Background Pattern */}
      <div className="absolute inset-0 opacity-5" style={{ backgroundImage: 'radial-gradient(#fb923c 1px, transparent 1px)', backgroundSize: '32px 32px' }}></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {steps.map((step, index) => (
              <div key={index} className="relative z-10 group">
                {/* Card Container */}
                <div className="bg-slate-950 border border-slate-800 rounded-xl p-6 h-full hover:border-orange-500/50 transition-all hover:-translate-y-2 hover:shadow-xl hover:shadow-orange-900/10">
                  
                  {/* Icon Circle */}
                  <div className="w-14 h-14 mx-auto bg-slate-900 rounded-full border border-slate-700 flex items-center justify-center mb-6 group-hover:bg-orange-600 group-hover:border-orange-500 transition-colors shadow-lg">
                    <step.icon className="h-6 w-6 text-slate-300 group-hover:text-white transition-colors" />
                  </div>
        ...
        <div className="mt-16 bg-slate-800/50 rounded-2xl p-2 border border-slate-700/50 max-w-5xl mx-auto backdrop-blur-sm">
            <div className="bg-slate-950 rounded-xl border border-slate-800 overflow-hidden relative aspect-16/7">
                 {/* Fake Dashboard UI using CSS only for static representation */}
                 <div className="absolute inset-0 flex flex-col">
                    ...
                    <div className="h-12 border-b border-slate-800 bg-slate-900 flex items-center px-4 justify-between">
                        ...
                        <div className="text-xs text-slate-500 font-mono">Vendor Dashboard - Admin Panel</div>
                    </div>
                    {/* Fake Body */}
                    <div className="flex-1 flex">
                        {/* Sidebar */}
                        <div className="w-48 border-r border-slate-800 bg-slate-900/50 hidden md:block p-4 space-y-3">
                        ...
                        <div className="flex-1 p-6 grid grid-cols-3 gap-4">
                            ...
                            <div className="bg-slate-900 rounded-lg p-4 border border-slate-800">
                                ...
                                <div className="text-white text-2xl font-bold">ETB 4,500</div>
                            </div>
                            {/* Card 3 */}
                            <div className="bg-slate-900 rounded-lg p-4 border border-slate-800">
                            ...
                            {/* List */}
                            <div className="col-span-3 bg-slate-900 rounded-lg border border-slate-800 h-full mt-2 p-4 space-y-3 opacity-50">
                 ...
                 <div className="absolute inset-0 flex items-center justify-center bg-black/40 backdrop-blur-[1px]">
                     <p className="text-white font-mono text-lg border border-white/20 px-4 py-2 rounded bg-black/50">
