...
export default function CartPage() {
  ...
  const fetchCart = async () => {
    try {
      const res = await axios.get(`${API_URL}/customer/getCart`, { withCredentials: true });
  ...
  const handleUpdateCart = async (itemId: string, newQty: number) => {
    ...
    try {
      await axios.put(`${API_URL}/customer/updateCart/${itemId}`, { quantity: newQty }, { withCredentials: true });
      await fetchCart();
    } catch (error) {
  ...
  const handleRemoveItem = async (itemId: string) => {
    setUpdatingId(itemId);
    try {
      await axios.delete(`${API_URL}/customer/removeFromCart/${itemId}`, { withCredentials: true });
      await fetchCart();
    } catch (error) {
  ...
  const handleClearCart = async () => {
    ...
    setUpdatingId('clear');
    try {
      await axios.delete(`${API_URL}/customer/deleteCart/${cart.restaurantId._id || cart.restaurantId}`, { withCredentials: true });
      setCart(null);
      setCount(0);
    } catch (error) {
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <DeliveryInput 
              ...
              onZoneSelect={setSelectedCoordinates}
            />
          </div>
...
function EmptyCart() {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-white dark:bg-slate-950">
      ...
      <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-3">Your cart is empty</h2>
      <Link href="/customer/restaurant" className="px-8 py-3 bg-orange-500 hover:bg-orange-600 text-white rounded-full font-bold">
        Browse Restaurants
      </Link>
    </div>
  );
